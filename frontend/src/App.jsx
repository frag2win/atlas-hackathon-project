  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `https://atlas-hackathon-project.onrender.com/${endpoint}`,
        {
          method: 'POST',
    ...
    if (activeTab === 'analyst') {
      return (
        <div className="space-y-3 text-gray-200 leading-relaxed">
          ...
          {results.osint_report &&
            results.osint_report.split('\n').map((p, i) => {
              if (p.trim().match(/^\d\./)) {
    ...
    return (
      <div className="space-y-3 text-gray-200 leading-relaxed">
        ...
        {results.final_synthesis && (
          <div>
            <h3 className="font-semibold">Moderator Synthesis</h3>
            {results.final_synthesis
              .split('\n')
          ...
          <div>
            ...
            {Object.entries(results.debate_transcript).map(([role, statement]) => (
              <div key={role} className="mb-2">
                <h4 className="text-indigo-400">{role.replace(/_/g, ' ').toUpperCase()}</h4>
                <p>{statement}</p>
              </div>
            ))}
          </div>
        ...
        {results.audit_report && (
          <div>
            <h3 className="font-semibold">Bias Audit Report</h3>
            {results.audit_report
              .split('\n')
              .map((p, i) => p.trim() && <p key={i}>{p}</p>)}
          </div>
    ...
    <div className="flex min-h-screen bg-gray-950 text-white font-sans">
        ...
        <div className="w-12 h-12 rounded-full bg-indigo-600 flex items-center justify-center mb-10 shadow-lg">
          ...
          >
            <path
              ...
              d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.5 3-9s-1.343-9-3-9m-9 9a9 9 0 019 9m-9-9a9 9 0 009-9m-9 9h12"
            />
          </svg>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center p-4 md:p-8">
        <header className="text-center mb-8">
          <h1 className="text-4xl md:text-5xl font-bold tracking-tight">ATLAS</h1>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    required
                    className="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-md shadow-sm py-2 px-3 text-white placeholder-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                    placeholder={
                      activeTab === 'analyst'
                        ? 'e.g., The future of quantum computing'
                        : 'e.g., Should AI weapons be banned?'
                    }
                  />
                </div>
                <div>
